...
export const removeSocketFromConnections = (connections: SocketConnection[], socket: Socket) => {
  ...
  if (userId) {
    ...
    if (connection) {
      ...
      if (shouldDeleteConnection) {
        const connectionIndex = connections.findIndex((connection) => connection.userId === userId);
