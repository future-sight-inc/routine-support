...
describe("getMonthsOfWeek", () => {
  it("Week of one month", () => {
    const week = createMockWeekFormDateStringArray(WEEK_OF_MONTH);
  ...
  it("Week between two months", () => {
    const week = createMockWeekFormDateStringArray(WEEK_BETWEEN_MONTHS);

    expect(getDayNumbersFromWeek(week)).toStrictEqual([28, 1, 2, 3, 4, 5, 6]);
  });

  it("Week between two months", () => {
    const week = createMockWeekFormDateStringArray(WEEK_BETWEEN_YEARS);
    expect(getDayNumbersFromWeek(week)).toStrictEqual([
      27, 28, 29, 30, 31, 1, 2,
    ]);
  });
