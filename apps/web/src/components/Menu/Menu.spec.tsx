...
describe("Menu", () => {
  it("Should be closed", () => {
    const { queryByTestId } = render(
      <AppWrapper>
        <Menu isOpened={false} options={[]} onClose={() => null} onSelect={() => null}>
          Anchor element
        </Menu>
      </AppWrapper>
    );
  ...
  it("Should display proper items", () => {
    const { queryByTestId } = render(
      <AppWrapper>
        <Menu isOpened options={[OPTION1, OPTION2]} onClose={() => null} onSelect={() => null}>
          Anchor element
        </Menu>
      </AppWrapper>
    );
    ...
    expect(queryByTestId(createOptionDataTestId(OPTION1.value))).toHaveTextContent(OPTION1.text);
    expect(queryByTestId(createOptionDataTestId(OPTION2.value))).toHaveTextContent(OPTION2.text);
  });
  ...
  it("Should display selected item", () => {
    ...
    expect(queryByTestId(MenuLocators.SelectedOption)).toHaveTextContent(OPTION1.text);
  });
});
