...
describe("getDateInfoFromMoment", () => {
  it("Regular week", () => {
    const mondayDateString = WEEK_OF_MONTH[0];
    const mondayMoment = parseDate(mondayDateString);

    expect(getDateInfoFromMoment(mondayMoment)).toStrictEqual(WEEK_OF_MONTH_INFO);
  });
  ...
  it("Week between months", () => {
    ...
    expect(getDateInfoFromMoment(mondayMoment)).toStrictEqual(WEEK_BETWEEN_MONTHS_INFO);
  });
  ...
  it("Last week of year", () => {
    ...
    expect(getDateInfoFromMoment(mondayMoment)).toStrictEqual(WEEK_BETWEEN_YEARS_INFO);
  });
  ...
  it("First week of year", () => {
    ...
    expect(getDateInfoFromMoment(mondayMoment)).toStrictEqual(FIRST_WEEK_OF_YEAR_INFO);
  });
});
