...
describe("getMonthsOfWeek", () => {
  it("Week of one month", () => {
    ...
    expect(getDayNumbersFromWeek(week)).toStrictEqual(DAY_NUMBERS_OF_WEEK_OF_MONTH);
  });

  it("Week between two months", () => {
    const week = createMockWeekFromDateStringArray(WEEK_BETWEEN_MONTHS);

    expect(getDayNumbersFromWeek(week)).toStrictEqual(DAY_NUMBERS_OF_WEEK_BETWEEN_MONTHS);
  });

  it("Week between two months", () => {
    const week = createMockWeekFromDateStringArray(WEEK_BETWEEN_YEARS);

    expect(getDayNumbersFromWeek(week)).toStrictEqual(DAY_NUMBERS_OF_WEEK_BETWEEN_YEARS);
  });
});
