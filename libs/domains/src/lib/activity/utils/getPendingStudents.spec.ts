...
describe("getPendingStudents", () => {
  it("No pending students", () => {
    ...
    expect(getPendingStudents(assignedStudents, confirmedStudents)).toStrictEqual([]);
  });
  ...
  it("One pending student", () => {
    ...
    expect(getPendingStudents(assignedStudents, confirmedStudents)).toStrictEqual([student2]);
  });
  ...
  it("All students are pending", () => {
    ...
    expect(getPendingStudents(assignedStudents, confirmedStudents)).toStrictEqual(assignedStudents);
  });
});
